<template>
  <div>
    ...
    <el-form :inline="true" :model="searchForm" class="demo-form-inline" label-width="100px">
        ...
        <el-form-item label="刊载状态">
          <el-input v-model="searchForm.kanzaizhuangtai"></el-input>
        </el-form-item>
      <el-form-item>
...
<!--      <el-table-column prop="bianjizhanghao" label="编辑账号" width="100"></el-table-column>
      ...
      <el-table-column prop="luyongshijian" label="录用时间" width="150"></el-table-column>
      <el-table-column prop="kanzaizhuangtai" label="刊载状态" width="100"></el-table-column>
      <el-table-column prop="shhf" label="审稿回复" width="100"></el-table-column>
      <el-table-column label="操作" fixed="right" width="200">
        <template slot-scope="scope">
          <el-button size="mini" v-if="flag==='shengaoren'" type="primary" @click="handleEdit(scope.row)">修改</el-button>
          <el-button size="mini" v-if="flag==='zuozhe'" type="primary" @click="kanzai(scope.row)">刊载</el-button>
          <el-button size="mini" v-if="flag==='shengaoren'" type="danger" @click="handleDelete(scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页 -->
...
<!--        <el-form-item label="编辑账号" prop="bianjizhanghao">
        ...
        <el-form-item label="录用时间" prop="luyongshijian">
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      this.$http.get('/gaojianluyong/' + (this.flag === 'zuozhe' ? 'page' : 'list'), {params})
          .then(response => {
    ...
    saveEdit() {
      this.$http.post('/gaojianluyong/update', this.form)
          .then(response => {
    ...
    handleDelete(row) {
      this.$confirm('此操作将永久删除该稿件, 是否继续?', '提示', {
        ...
        this.$http.post('/gaojianluyong/delete', [row.id])
            .then(response => {
              this.$message({
                type: 'success',
                message: '删除成功!'
